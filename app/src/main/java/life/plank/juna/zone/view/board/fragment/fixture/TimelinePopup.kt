...
import life.plank.juna.zone.view.board.adapter.match.TimelineAdapter
...
class TimelinePopup : BaseBlurPopup() {
    ...
    private fun prepareViews() {
        ...
        time_status.text = matchDetails.timeStatus?.run { MatchDataService.getDisplayTimeStatus(this) } ?: NOT_STARTED
        date.text = getTimelineDateHeader(matchDetails.matchStartTime)
        score.text = MatchDataService.getSeparator(matchDetails, win_pointer, false)
    ...
    private fun setZoneLiveData(intent: Intent) {
        val zoneLiveData = MatchDataService.Live.getZoneLiveData(intent, getString(R.string.intent_zone_live_data), gson)
        when (zoneLiveData!!.liveDataType) {
            SCORE_DATA -> {
                val scoreData = zoneLiveData.getScoreData(gson)
                MatchDataService.Live.updateScoreLocally(matchDetails, scoreData)
            ...
            TIME_STATUS -> {
                ...
                time_status.text = MatchDataService.getDisplayTimeStatus(matchDetails.timeStatus!!)
            }
        }
    }
