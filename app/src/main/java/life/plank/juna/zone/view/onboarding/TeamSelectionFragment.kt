package life.plank.juna.zone.view.onboarding
...
import life.plank.juna.zone.view.base.fragment.SearchableCard
...
class TeamSelectionFragment : SearchableCard(), View.OnTouchListener {
    ...
    private fun getPopularTeams() {
        if (isNullOrEmpty(IdToken))
            return
        restApi.getPopularTeams().setObserverThreadsAndSmartSubscribe({
    ...
    private fun postTeamPref(zone: String) {
        if (onBoardingAdapter?.selectedTeamNames?.isEmpty() == false) {
            restApi.postTeamPreferences(zone, onBoardingAdapter!!.selectedTeamNames).setObserverThreadsAndSmartSubscribe({
    ...
    private fun getFootballTeams(teamName: String) {
        searchSubscription = restApi.getSearchedFootballTeams(teamName).setObserverThreadsAndSmartSubscribe({
