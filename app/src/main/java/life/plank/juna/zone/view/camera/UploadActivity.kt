...
package life.plank.juna.zone.view.camera
...
class UploadActivity : AppCompatActivity() {
    ...
    fun initListeners() {
        post_btn.setOnClickListener {
            ...
            when (openFrom) {
                IMAGE, GALLERY -> postMediaContent(getString(R.string.media_type_image), IMAGE, getRequestDateStringOfNow())
                VIDEO -> postMediaContent(getString(R.string.media_type_video), VIDEO, getRequestDateStringOfNow())
                AUDIO -> postMediaContent(getString(R.string.media_type_audio), AUDIO, getRequestDateStringOfNow())
    ...
    private fun updateUI(type: String) {
        ...
        Glide.with(this)
                .load(CurrentUser.profilePicUrl)
    ...
    private fun prepareImageForUpload() {
        try {
            ...
            if (imgFile.exists()) {
                val bitmap = ImageCompressor().compress(imgFile, filePath)
    ...
    private fun postMediaContent(mediaType: String, contentType: String, dateCreated: String) {
        ...
        restApi.postMediaContentToServer(body, boardId, contentType, userId!!, dateCreated, AppConstants.BOARD, title_text.text?.toString()!!.trim())
